...
# combining walk commute trips with intrazonal trips
...
# combining bike commute trips with intrazonal trips
...
# READ IN LINK DATA
bikeLinksShort <- left_join(AllBikeShort, networkBike)
bikeLinksFast <- left_join(AllBikeFast, networkBike)
walkLinksShort <- left_join(AllWalkShort, networkBike)
walkLinksFast <- left_join(AllWalkFast, networkBike)
bikeLinksShort2 <- readr::read_csv("data/manchester/AllBikeShort.csv") %>% left_join(networkBike)
...
bikeAggShort = bikeLinksShort2 %>% aggregateBE(length,"bike")
...
# Save to csv
write_csv(resultShort,"data/manchester/commuteShort92.csv")
write_csv(resultFast,"data/manchester/commuteFast92.csv")